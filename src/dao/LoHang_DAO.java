...
public class LoHang_DAO {
    ...
    public String getNextID(){
        ...
        String sql =
                "SELECT MAX(CAST(SUBSTRING(Ma_Lo,3,6) AS INT)) FROM LOHANG";
    ...
    public boolean them(LoHang_DTO lo){
        ...
        String sql =
                "INSERT INTO LOHANG(" +
                        "Ma_Lo,GiaNhap,HSD," +
                        "SoLuong,SoLuongConLai,SoLuongSPCL,ThanhTien," +
        ...
        try(Connection con = DBConnection.getConnection();
            ...
            ps.setInt(6,lo.getSoLuongSPCL());

            ps.setDouble(7,lo.getThanhTien());
    ...
    public boolean capNhat(LoHang_DTO lo){
        ...
        String sql =
                "UPDATE LOHANG SET " +
                        "GiaNhap=?,HSD=?," +
                        "SoLuong=?,SoLuongConLai=?,SoLuongSPCL=?," +
                        "ThanhTien=?," +
                        "TrangThai=?,TrangThaiTonKho=?," +
                        "Ma_PNK=?,Ma_NCC=?,Ma_KVLT=?,Ma_SP=? " +
                        "WHERE Ma_Lo=?";
    ...
    public boolean capNhatTrangThai(String maLo,int trangThai){
        ...
        String sql =
                "UPDATE LOHANG SET TrangThai=? WHERE Ma_Lo=?";
